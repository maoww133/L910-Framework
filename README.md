...
Пьесы (plays.json): id (string), title (string), duration (number), isClassic (boolean), premiereDate (string как Date), genres (array).
Представления (performances.json): id (string), playId (string), date (string как Date), seatsAvailable (number), isSoldOut (boolean), actors (array).
Роутинг: Plays
